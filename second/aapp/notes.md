...
Reduce is used to **combine a collection of elements into one summary value**. A
**combiner function** is an **associative** function that **combines elements
pairwise**. Reduction can be **done in a parallel way** (breaking up the
collection in a binary-tree like fashion) **or in a vectorized way** (vector
instructions serially executed). By employing **tiling we can breakup the list
into multiple chunks for workers to reduce serially**. **Map and reduce can be
fused into a single operation**.

Precision can become a problem with reductions on floating point data. Different
orderings of floating point data can change the reduction value.

### In detail: scan

The scan pattern **produces partial reductions of the input sequence, generating
a new sequence**. We have **two ways** of performing a scan:

1. **Inclusive** scan: includes current element in partial reduction
2. **Exclusive** scan: excludes current element in partial reduction, partial
   reduction is of all prior elements prior to current element

One **algorithm for parallelizing scan is to perform an "up sweep" and a "down
sweep"**: in the **"up sweep" we reduce the input**, on the **down sweep we
compute the intermediate** variables.

Like with reduction, we can **employ tiling to breakup the operation into serial
chunks.** Also like reduction, we can **fuse map and scan**.

### In detail: gather

The gather pattern **creates a (source) collection of data by reading from
another (input) data collection**: given a **collection of (ordered) indices,
read data from the source at each index and write it to the output collection in
index order**. This leaves us with an output that is of the same type as the
source and the same shape as the input.

There are some **special cases** like:

1. **Shifts**: moves data **one position to left/right**
   - Index array is implicit
   - Elements that fall outside the array are discarded, new elements can be
     generated by duplicating the last element of rotating (moving the first
     element in the last position)
2. **Zip**: **interleaves** two arrays
3. **Unzip**: reverses a zip

### In detail: scatter

Scatter is the **inverse to gather**: we **take an array of values** the same
shape of the index array and we **use the index array to know where to write in
an output collection**. This can lead to **race condition due to concurrent
updates** of the same variable. To solve this collisions, we have different
methodologies to ensure correctness:

1. **Atomic scatter**: only one of the possible values will be written entirely
   **(non-deterministic approach)**
2. **Permutation scatter**: make **collisions illegal**
   - We need to check for collisions in advance
3. **Merge scatter**: apply a **commutative-associative operator to merge**
   colliding values
4. **Priority scatter**: **every element in the input array is assigned a
   priority** based on its position, this **priority is used to decide which
   element wins** in case of collisions

**Scatter has worse performance than gather** since we may thrash the cache. If
we can convert it into a gather, we can improve performance. However it cannot
always be done.

### In detail: pack

Given **two collections** (one with data and one with booleans), **keep only the
elements which have a `true` in the boolean array**. This basically **moves
elements such that they are in contiguous memory**.

To **implement** pack we can do:

1. **Map to convert bools to** `0` and `1`
2. **Exclusive scan** to compute the **cumulative sums**, this will be our
   **write offsets**
3. **If in the initial bool array we have** `true`, we **write the element at
   the offset calculated in step 2**

**Unpack is the inverse of pack**. We can **implement** it as such:

1. We **calculate an exclusive scan of the bool array**
2. We apply a **scatter** (no collisions are possible) **checking before the
   status of the bool**

Some **generalization** of the pack pattern:

1. **Split**: elements are **moved to upper or lower half of output collection
   based on the boolean value**
   - More or less like before, but we need to do two steps: one for the zeroes
     and one for the ones. The we do two moves
2. **Unsplit**: reverse of split
3. **Bin**: **generalized split** to support more categories

**Map and pack can be combined into a single operation**: **map** checks the
element and **calculates if it should be discarded**. We can **generalize** this
"fusion" into the **expand pattern**: **each element can output any number of
elements, results are then fused together in order**.

### Array of Structures vs Structure of Arrays

The **first** organization **may lead to better cache utilization if data is
accessed randomly**. The **second is usually better for vectorization** and
avoiding the "false sharing" problem: when a system participant **attempts to
periodically access data that is not being altered** by another party, but that
**data shares a cache block with data that is being altered**, the caching
protocol **may force the first participant to reload** the whole cache block
**despite a lack of logical necessity**.

Depending on the two, we have **different data layout options**:

1. **Array of structures**:
   - Put **padding at the end**
   - Put **padding after each structure**, enough to align it with the closest
     boundary
2. **Structure of arrays**:
   - Put **padding at the end**
   - Put **padding after each component**, enough to align it with the closest
     boundary
...
We can see that the **AoS implementation is the most logical**, however it has
**pretty difficult memory access for reads and writes, preventing efficient
vectorisation**. **Separate arrays** for each field, like in the SoA case,
**keeps memory accesses contiguous when vectorization is performed** over
structure instances.
